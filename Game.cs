...
public class Game
{
    ...
    public static byte _turnToken = 1;
...
/*    public static int PlayerChoice()
    ...
    public static void AdvanceTurn(ref byte TurnToken)
    {
        if (TurnToken % 2 == 1)
            TurnToken = 2;
        else TurnToken = 1;
    ...
    public static void EnemyLogic(ref byte turnToken, out string response)
    {
        ...
        switch(decision)
        {
            case 1: Actions EnemyAttack = new("Attack", ref turnToken, out response); break;
            case 2: if (EnemyHP <= 15) { Actions EnemyHPRegen = new("RegenHP", ref turnToken, out response); break; } else goto case 1;
            case 3: if (_enemyMana <= 5) { Actions EnemyManaRegen = new("RegenMana", ref turnToken, out response); break; } else goto case 1;
        }
        turnToken = 1;
    }
    static Game()
    {
        Difficulty diff = new();
        PlayerHP = (int)(Math.Round(Math.Max(12, RollDice(3, 15)) * diff.DifficultyModPlayer));
        EnemyHP = (int)(Math.Round(Math.Max(12, RollDice(3, 15)) * diff.DifficultyModEnemy));
        _playerMana = (int)(Math.Round(Math.Max(4, RollDice(2, 6)) * diff.DifficultyModPlayer));
        _enemyMana = (int)(Math.Round(Math.Max(4, RollDice(2, 6)) * diff.DifficultyModEnemy));
